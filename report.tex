...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{./images/BRAM APP.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{./images/Iterations counter.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{./images/Operating Mode Selection.png}
...
\qquad Khối VTC calculation có nhiệm vụ cốt lõi là tính toán các thông điệp từ nút biến đến nút kiểm tra VTC. Đầu vào của quá trình tính toán này là các giá trị xác suất hậu nghiệm APP được cung cấp từ khối lưu trữ BRAM APP, kết hợp với các thông điệp CTV từ vòng lặp trước. Mục tiêu kết quả đầu ra là khối phải xử lý mượt mà các dữ liệu số có dấu để phản ánh đúng tính chất của các quyết định mềm trong giải mã LDPC. Khối có một số đặc điểm nổi bật sau:
\begin{itemize}
    \item Tham số hóa: Thiết kế sử dụng tham số $W = 8$ bit, cho phép hệ thống linh hoạt thay đổi độ rộng của bus dữ liệu đầu vào và đầu ra. Điều này rất hữu ích khi cần cân bằng giữa độ chính xác của giải mã và tài nguyên phần cứng như diện tích mạch, công suất tiêu thụ...
    \item Bộ trừ có dấu: Thành phần toán học duy nhất và cốt lõi của khối là một bộ trừ. Thay vì sử dụng các IP lõi phức tạp, khối sử dụng trực tiếp phép gán liên tục qua lệnh \textit{assign vtc\_out = app\_in - ctv\_in;} để phần mềm synthesis tự động ánh xạ xuống cấu trúc mạch trừ tối ưu nhất trên FPGA.
    \item Cổng I/O: Khối bao gồm hai bus đầu vào \textit{app\_in}, \textit{ctv\_in} và một bus đầu ra \textit{vtc\_out}. Tất cả đều được ép kiểu logic signed có độ rộng $W$ bit để đảm bảo phép toán bù 2 được thực thi chính xác đối với các giá trị âm.
\end{itemize}

\qquad Luồng dữ liệu của khối VTC hoạt động theo cơ chế luân chuyển liên tục như sau:
\begin{itemize}
    \item Truy xuất dữ liệu định tuyến: Để luồng tính toán diễn ra chính xác, hệ thống phải sử dụng một ma trận kiểm tra được lưu trữ trong bộ nhớ phân tán. Thông tin từ bộ nhớ này đóng vai trò quyết định trong việc lấy đúng các giá trị cấu thành từ khối BRAM APP để đưa vào tính toán các thông điệp VTC tương ứng.
    \item Thực thi biểu thức toán học: Ngay khi dữ liệu được nạp vào, bộ trừ logic tổ hợp sẽ thực hiện biểu thức tính toán VTC rút gọn~\ref{eq:2}.
    \item Chuyển tiếp luồng: Do tính chất của mạch tổ hợp, giá trị \textit{vtc\_out} được tạo ra gần như tức thời, chỉ trễ một khoảng thời gian truyền qua các cổng logic gate delay. Dữ liệu này ngay lập tức được đẩy liên tục sang khối Min/submin calculation để tiếp tục quy trình tìm kiếm giá trị cực tiểu trong vòng lặp giải mã hiện tại.
\end{itemize}
